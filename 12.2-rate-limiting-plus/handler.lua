...
function plugin:access(conf)
    ...
    --构建缓存key
    local cache_key_str = cache_key(string.format("%s_%s_%s_%s", host, path, method,company_id))
    --构建查询限流服务器数据来源的请求参数
    local query = string.format("domain=%s&path=%s&method=%s&companyId=%s", host, path, method,company_id)
    --查询并缓存在mlcache
